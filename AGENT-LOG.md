...
## 2026-09-07 10:40 UTC — run 98: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 97 (`a625be3`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The price breakdown (`#price-toggle` / `#breakdown-body`) — not
clicked by hand in a while. Built a door with a vertical slot window,
Cadoor knob and bronze פרזול (`?w=strip&n=cadoor&pz=pz-bronze`),
expanded the breakdown, and added the nine rows by hand: 1295 + 200 +
200 + 500 + 700 + 300 + 4200 + 200 + 500 = ₪8,095, matching the total
exactly. `aria-expanded` flipped correctly, the card renders clear of
the door and sconces, and the bronze finish showed correctly on both
the knob and the cylinder rim. No console errors.
...
**Commit:** 8dc92d3a3b879b6aa74a4136f8cd4eabe33c2819
...
The classical set (`d=classic`) in its solid variant (default
`window: none`, English UI) — not recently exercised. Confirmed the
set draws correctly: cornice, corbelled shelf with its own turned pull,
raised panel where the light would go, plinth, all proportioned to the
leaf rather than to a generic moulding. Spec correctly reads
"Window: Solid" since the classical set supports both a solid and a
glazed variant and no `w=` was given. English interface stayed LTR
while the drawing stayed square-on and unmirrored, as required. ₪5,895,
no console errors.
...
**Commit:** 34c87253c1ce9f2c27ed307e9249da108f424893
...
**Commit:** 53ed7688bf3d7724e235897ab9a20068fc2677b2
...
**Commit:** 35d02704d2362bae727af84abda4cc6a3e9b732d
...
**Commit:** 3fd14d5a3715902281ffca5b5568dac39b55c162
...
**Commit:** a211940465ab029b5ef6091f24c958f4bfdb678c
...
The grape-and-vine etched glass pattern over a vertical slot window
(`?w=strip&g=vine&c=rb-9001d`), not recently exercised — winding vine
with grape clusters etched into the pane, correctly a worked-glass
pattern (in the pane, not bolted over it). Spec row reads "גפן".
₪8,095, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
89) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
...
**Commit:** 334db6742a46184d89786c0af73c4bc64c7f344e
...
The art-deco geometric-lines grille over a vertical slot window
(`?w=strip&g=deco&c=rb-9016d`), not recently exercised — the d123
pattern (horizontal bars at the bottom third with a vertical member)
drawn correctly, spec row reads "סורג: קווים גיאומטריים". ₪7,395, no
console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
87) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-05 05:40 UTC — run 87: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 86 (`b26050e`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The interlocking-rings grille etched into a rectangular window
(`?w=broad&g=rings&c=rb-9016d`), not recently exercised — `broad` is a
withdrawn window id and correctly aliased onto `rect` (spec row reads
"חלון: חלון מלבני", not "broad"), the ring lattice drawn correctly, and
the forced bottom panel appeared with its repair notice ("square window
always comes with one"), matching the documented `rectNeedsPanel` rule.
₪6,995, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
86) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-05 00:40 UTC — run 86: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 85 (`fd4fbcc`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The Nitzan pull bar on an oversize door (`?n=nitzan&s=extra1&c=rb-7126d`),
not recently exercised — round tube with correct wrap-around shading,
spec row reads "ידית משיכה: ניצן · 100 ס״מ" (its own measured length).
₪4,495, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
85) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
The Cadoor round-escutcheon lockset with a gold פרזול (`?n=cadoor&pz=pz-gold`),
not recently exercised — cropped a 3x screenshot of the fitting itself:
the rose correctly reads gold while the ball knob at its centre stays
its own fixed dark tone, matching the documented rule that the Cadoor's
ball (like the Sapir's mirror knob) is measured off the real product and
wired to no finish at all. ₪4,295, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
83) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
The recessed channel handle (`?n=channel`, `ידית שקועה`), not recently
exercised — drawn correctly as a groove cut into the leaf rather than a
bolted-on bar, spec row reads "ידית שקועה", ₪4,895. Attempted a drag on
it (mouse down/move/up over its hitpad) and confirmed the resulting
send link's href was byte-identical before and after — matching the
documented `fixed: true` rule ("it stays in the normal spot", read by
`armGrip`, which does not arm the drag for this fitting). No console
errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
80) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-03 20:40 UTC — run 80: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 79 (`052d4e4`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

Continued exercising the live stripe controls (not the `?d=` legacy
migration path, per run 78/79's note): stepped horizontal spread
stripes up to its max of 11 (the `+` button correctly disables there),
then switched to the tight toggle — the count correctly re-clamped down
to 8 and the `+` button correctly disabled again at the new max. Eight
tight stripes drawn across the middle band, "צפופים" pill shown on,
stripe line-item ₪1,200 (8 × ₪150 horizontal rate), ₪4,595 total, no
console errors. Confirms both the spread/tight max-count clamp and the
re-clamp-on-toggle logic in `js/app.js`'s stripe builder.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
79) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
The ragged nine-strip family (`?d=strips9&c=rb-7021d`), not recently
exercised — anchored at the hinge stile, free ends of varying length,
correctly distinct from the tight/spread band families. Spec row reads
"פסים: 9 אופקיים" with no tight qualifier. ₪4,745, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
76) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-03 00:40 UTC — run 76: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 75 (`f6cb66c`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The light-coloured scroll grille over a rectangular window
(`?w=rect&g=scroll-light&c=rb-9016d`), not recently exercised — ornate
scrollwork drawn in the door's own paint rather than black, and the
forced bottom panel (`rectNeedsPanel`) correctly appeared free of
charge with a repair notice explaining why. ₪6,995, spec row reads
"סורג: מעוצב בהיר", no console errors.

⚠ The container restarted mid-run, taking the backgrounded `npm test`
and `npm run audit` with it. The working tree survived intact (clean,
same commit) and the four already-completed instruments
(`collide -- all`, `collide -- boxes`, `profile`, `recreate`) needed no
re-run since their results were already captured; `test` and `audit`
were relaunched from scratch and both came back clean.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
75) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-02 20:40 UTC — run 75: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 74 (`d125ef3`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The tight horizontal band (`?d=stripsband&c=rb-0097d`), not recently
exercised — eight fine lines across the middle fifth of the leaf, spec
row correctly reads "פסים: 8 אופקיים · צפופים" (tight, not the spread
ragged family). ₪4,595, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
74) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
## 2026-09-02 15:40 UTC — run 74: nothing worth changing

**Looked at:** `git fetch` — no new commits since run 73 (`211bcf9`
still the tip on both sides). `npm run build` produced no diff, bundle
already matched source.

The fanned vertical stripes (`?d=stripsv6&c=rb-9016d`), not recently
exercised — six bands off a common foot, climbing to different heights
as documented (`STRIP_V`'s fanned family). Spec row reads "פסים: 6
אנכיים", ₪4,995, no console errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
73) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
The classical set (`?d=classic&w=rect&c=rb-9016d`) at 1280x900, not
recently exercised — cornice, frieze block, the glazed light cased
correctly inside its own architrave with no overlap into the frieze or
shelf, corbelled shelf with its turned pull, panel, plinth, all in
white paint. ₪7,895, spec row reads "עיצוב: סט קלאסי", no console
errors.

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
71) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
...
Two things not recently exercised: the widest דו כנפי (`s=halfextra2`)
with a matching vertical-slot window on both leaves at 1440x900 — main
leaf and side leaf both correctly glazed, side leaf's window correctly
narrower ("חלון צר תואם"), price ₪13,785, handing card correct, no
console errors. Then the undo/redo pair in the wall: changed colour
(₪13,785 → ₪13,985), changed lock (→ ₪14,185), undo twice (→ ₪13,985 →
₪13,785, button correctly disabled once the stack is empty), redo once
(→ ₪13,985). All prices round-tripped exactly. (My first pass guessed
the wrong selector for the undo button and silently no-opped — corrected
to `#undo-btn`/`#redo-btn` from `index.html`, not a site issue.)

**Instruments:** test ✓ (4,349,513 passed, 0 failed — identical to run
69) · audit ✓ (0 faults, all eight viewports) · profile ✓ (both FALLOFF
bands and all four bead ratios within tolerance, byte-identical) ·
collide -- all ✓ (1,902 designs, faceObstacles agrees with the drawing
everywhere) · collide -- boxes ✓ (deepest bolted lock furniture 111 mm
against MOUNT_REACH 121) · recreate ✓ (10 doors, same catalogue-gap
