...
function handleOf(rec) {
  ...
  /* ⚠ THE FINISH IS AN AXIS NOW, NOT A FILTER — 20.9.2026. For a round it
     ...
     defined by their metal (the brass אלה, the black מוט שחור) and a black
     bar on a chrome door is wrong at any length; that term is what stopped
     `barblack` winning on geometry for four steel doors, and what put the
     one black bar we sold — 800 mm — on d072's man-high bar until it became a
     term rather than a veto. Both bars left the range today and their metals
     became `handleFinish`, a field of the state. So the SECTION is matched on
     geometry alone between the two bars that survive, and the record's finish
     is read straight into the field — which is what the record was saying all
     along, and what the fitter had to approximate while a finish was a
     product. `family` is the same reading of the free-text finish it was. */
  const family = f => /black|שחור/.test(String(f || '')) ? 'hf-black'
                    : /brass|bronze|gold|פליז/.test(String(f || '')) ? 'hf-gold' : 'hf-nickel';
  let best = null, bestD = Infinity;
   ...
   catches one of them being regenerated without the other. */
{
  ...
  /* ⚠ AND `handleFinish` IS ON THE LIST SINCE 20.9.2026, THE DAY IT WAS
     FOUND MISSING. The fitter above derived the finish for every bar and
     returned it; `want` carried it; the markdown table's query carried it —
     and this list, written by hand before the field existed, dropped it on
